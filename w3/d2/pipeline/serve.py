...
# ---------------------------------------------------------------------------
# Additional endpoints for chaos_runner compatibility
# ---------------------------------------------------------------------------

# In-memory alert storage for chaos experiments
_alert_store: list[dict] = []
_alert_store_lock = threading.Lock()


@app.get("/alerts", tags=["pipeline"])
def get_alerts(since: int) -> list[dict]:
    """
    Return alerts fired since the given timestamp.
    Used by chaos_runner to query pipeline state during experiments.
    """
    with _alert_store_lock:
        return [
            {**a, "fire_ts": a.get("fire_ts", int(time.time()))}
            for a in _alert_store
            if a.get("fire_ts", 0) >= since
        ]


@app.post("/rca", tags=["pipeline"])
def post_rca(request: dict) -> dict:
    """
    Run RCA for a given time window.
    Used by chaos_runner to get root cause analysis.
    """
    window_start = request.get("window_start", 0)
    window_end = request.get("window_end", int(time.time()))
    
    with _alert_store_lock:
        alerts_in_window = [
            a for a in _alert_store
            if window_start <= a.get("fire_ts", 0) <= window_end
        ]
    
    if not alerts_in_window:
        return {"root_service": "unknown", "confidence": 0.0}
    
    # Run pipeline on the alerts
    result = pipe.process_batch(alerts_in_window)
    
    return {
        "root_service": result["root_cause"]["service"],
        "confidence": result["root_cause"]["confidence"],
...
def ingest_alerts(req: IncidentRequest) -> dict:
    """
    Ingest alerts into the internal store for chaos experiments.
    This is a helper endpoint to simulate alert ingestion during chaos runs.
    """
    with _alert_store_lock:
        for a in req.alerts:
            alert_dict = a.model_dump()
            alert_dict["fire_ts"] = int(time.time())
            _alert_store.append(alert_dict)
    
    return {"status": "ingested", "count": len(req.alerts)}
