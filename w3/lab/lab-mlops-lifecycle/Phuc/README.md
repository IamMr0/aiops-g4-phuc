...
To run the pipeline from start to finish from the root of the lab (`lab-mlops-lifecycle`), follow these steps:
...
5. Simulate drift detection and automatic retraining by running:
   `export MLFLOW_TRACKING_URI=http://localhost:5000`
   `uv run python Phuc/retrain.py --reference data-pack/data/baseline.csv --current data-pack/data/drifted.csv --serve-url http://localhost:8000`
